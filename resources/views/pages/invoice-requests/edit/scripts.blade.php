<script>
    var lastProductId = document.getElementById('product_count').value;
    var productNumber = document.getElementById('product_count').value;
    var isValid=false
    ...
    function submitInvoice(id) {
        ...
        let  url = '{{ route("invoice.request.update", ":id") }}'.replace(':id', id);
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    Swal.fire({
                        title: "Success!",
                        text: "Invoice Request Saved Successfully.",
                        icon: "success",
    ...
    function checkQtysAndRate(){
        document.querySelectorAll('input[name="qty[]"]').forEach((input) => {
            const value = parseFloat(input.value); // current input value
            const maxQty = parseFloat(input.dataset.maxqty); // from data-maxqty
        ...
        document.querySelectorAll('input[name="rate[]"]').forEach((input) => {
            const value = parseFloat(input.value); // current input value
            const minrate = parseFloat(input.dataset.minrate); // from data-maxqty
