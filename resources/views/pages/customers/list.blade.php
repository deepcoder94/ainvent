...
<div>
    <button class="btn btn-primary mt-2 mb-2" onclick="showCreateForm()">Add New</button>  
    <div style="float: right;margin-top:10px;display:inline-flex">
    <input type="text" id="searchField" placeholder="Search Customer" onblur="searchCustomer()" style="width:unset;margin-right:11px" class="form-control">        
    <select class="form-control" id="beatSearch" style="width: unset;margin-right:11px" onchange="searchCustomer()">
        <option value="">Filter By Beat</option>
        @foreach ($beats as $b)
            <option value="{{ $b->id }}">{{ $b->beat_name }}</option>
        @endforeach
    </select>
    <a href="javascript:void(0)" class="btn btn-success" onclick="searchCustomer(true)">Clear Filter</a>
    </div>
</div>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <!-- Vertical Form -->
              <form class="row g-3" id="customer_form" action="#" method="post">
                <div class="col-12">
                  <label for="beat_id" class="form-label">Select Beat</label>
                ...
                <div class="col-12">
                  ...
                  <div class="col-12">
                    <label for="customer_gst" class="form-label">Customer Phone</label>
                    <input type="text" class="form-control" id="customer_phone" name="customer_phone" required>
                  </div>                                
                  <div class="col-12">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="customer_active" name="customer_active">
                        <label class="form-check-label" for="customer_active">Is Active</label>
                      </div>                  
    
                  </div>                      
                  <input type="hidden" id="store_url" value="{{ route('customer.store') }}">
            ...
            <div class="modal-footer">
              ...
              <button type="button" onclick="submitCustomerForm()" class="btn btn-primary">Save changes</button>
            </div>
          </div>
        </div>
      </div><!-- End Basic Modal-->  
...
function submitCustomerForm(){
        ...
        let data = {
            customer_name: form.find(item => item.name === 'customer_name').value,
            customer_address: form.find(item => item.name === 'customer_address').value,
            customer_gst: form.find(item => item.name === 'customer_gst').value,
            customer_phone: form.find(item => item.name === 'customer_phone').value,
...
Swal.fire({
    ...
    function searchCustomer(clearSearch){
        ...
        $.ajax({
            ...
            data: { searchField:$("#searchField").val(),beatSearch: $("#beatSearch").val() },
            success: function(response) {     
